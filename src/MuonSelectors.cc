...
bool muon::isTightMuonWithBS(const reco::Muon& muon, const reco::BeamSpot& beamSpot){
  ...
  /// sehwook test
  bool ip = fabs(muon.muonBestTrack()->dxy(beamSpot.position())) < 0.2 && fabs(muon.muonBestTrack()->dz(beamSpot.position())) < 0.5;
...
bool muon::isSoftMuonWithBS(const reco::Muon& muon, const reco::BeamSpot& beamSpot){
  ...
  bool ip = fabs(muon.innerTrack()->dxy(beamSpot.position())) < 3. && fabs(muon.innerTrack()->dz(beamSpot.position())) < 30.;
...
bool muon::isHighPtMuonWithBS(const reco::Muon& muon, const reco::BeamSpot& beamSpot){
  ...
  bool ip = fabs(muon.muonBestTrack()->dxy(beamSpot.position())) < 0.2 && fabs(muon.bestTrack()->dz(beamSpot.position())) < 0.5;
